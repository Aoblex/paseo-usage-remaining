...
The composer displays one compact capsule per available provider. Each capsule shows the provider logo and name once, then keeps its primary windows together as remaining percentage followed by reset countdown. Window lengths stay in the full dashboard instead of being repeated inline. Monetary balances show only the amount. Narrow clients wrap whole provider capsules when needed.
...
| Codex | Environment, Pi `openai-codex`, OpenCode `openai`/`openai-codex`, or Codex CLI (`~/.codex/auth.json`) | Shows only the account's primary rate-limit windows; model-specific `additional_rate_limits` are hidden. Pi's multi-account slots (`openai-codex`, `openai-codex-2`, …) each get their own `Codex #n` card and chip |
...
- Every Codex account slot in Pi's `auth.json` is one ChatGPT account with its own quota, so each becomes a numbered `Codex #n` card and chip. A slot whose stored token the API rejects keeps a dimmed `—` chip with the reason (`Rejected: Pi (401)`, `Query failed (401)`); re-run `/login` in Pi and pick that `OpenAI Codex #n` provider to restore it. A single-account install still renders as plain `Codex`.
- The composer pill label names the agent's own provider. Pi and OpenCode serve every configured model under one provider id, so the label resolves the brand from the agent's model instead.
