...
test('Kimi parses short and weekly quota windows plus extra balance', () => {
  ...
  assert.equal(rows.find((row) => row.id === 'kimi_session').metricLabel, '5-hour limit');
  assert.equal(rows.find((row) => row.id === 'kimi_week').remainingPct, 75);
  assert.equal(rows.find((row) => row.id === 'kimi_week').metricLabel, 'Plan limit');
...
test('GLM parses token/credit and MCP limits as remaining percentages', () => {
  ...
  assert.equal(rows[0].id, 'glm_credit_session');
  assert.equal(rows[0].metricLabel, '5-hour credit limit');
  assert.equal(rows[0].remainingPct, 82);
  assert.equal(rows[1].id, 'glm_mcp_weekly');
  assert.equal(rows[1].metricLabel, 'MCP limit');
